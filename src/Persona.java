...
public class Persona extends PersonaApp.PersonaPOA{
 ...
 boolean resultado = false;
    ...
    public boolean actualizarPersona(int id_persona, String nombre, String apellido, int telefono, String direccion, String correo) {
        boolean resultado = false;
        try {
            String query = "update persona set nombre = '"+nombre+"',apellido = '"+apellido+"' apellido = '"+telefono+"' telefono = '"+telefono+"' direccion = '"+direccion+"'correo = '"+correo+"' where id_persona = '"+id_persona+"' ";
            conex.conexion();
            Statement st = conex.conex.createStatement();
            int valor = st.executeUpdate(query);
            if(valor>0){
